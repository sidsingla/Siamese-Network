...
def _get_filenames_and_classes(dataset_dir):
  ...
  for subdir in os.listdir(dataset_dir):
    ...
    for fil in os.listdir( dataset_dir + '/' + subdir ):
      photo_filenames.append(fil)
...
def _convert_dataset(split_name, filenames, class_names_to_ids, dataset_dir, tfrecord_filename, _NUM_SHARDS, output_dir):
    ...
    with tf.Session('') as sess:
      ...
      for shard_id in range(_NUM_SHARDS):
        output_filename = _get_dataset_filename(
            output_dir, split_name, shard_id, tfrecord_filename = tfrecord_filename, _NUM_SHARDS = _NUM_SHARDS)
        ...
        with tf.python_io.TFRecordWriter(output_filename) as tfrecord_writer:
          ...
          for i in range(start_ndx, end_ndx):
            ...
            # Read the filename:
            try:
              subdir = filenames[i][:4]
              image_data = tf.gfile.FastGFile(dataset_dir + '/' + subdir + '/' + filenames[i], 'rb').read()
            ...
            example = image_to_tfexample(
                image_data, b'jpg', height, width, class_id, filenames[i].encode())
